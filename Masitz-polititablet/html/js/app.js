/* ============================================================
   kc_mdt · js/app.js
   Core application: state, login, navigation, panels.
   Depends on api.js (loaded first) and dispatch.js (for the
   Dispatch module) — both already on window when this runs.
   ============================================================ */
...
const state = {
    ...
    lawsCache: null,
    accountsCache: null,
    lawsView: 'laws',            // 'laws' | 'radio' | 'rights' | 'corruption' | 'weapons'
    lawsCategory: null,          // active book-filter in the laws category-nav
    chargesCategory: null,       // active category-filter in the charges category-nav
    rightsTab: null,
    onlinePlayersCache: null,
    quickSearchTimer: null,
    personSearchTimer: null,
    vehicleListTimer: null,
    caseSearchTimer: null,
    lawsSearchTimer: null,
    arrestForm: null,
};

// ─── OPACITY-SYSTEM (§1, §2 — MUST stay intact) ───────────────
...
function initOpacitySystem(uiCfg) {
    ...
    if (zone) {
        zone.onmouseenter = () => { opacityHovering = true; applyOpacityState(); };
        zone.onmouseleave = () => { opacityHovering = false; applyOpacityState(); };
    }
}
...
// ─── JOB / GRADE BADGE (§6 — erstatter det kedelige "-") ───────
const GRADE_TIERS = [
    { min: 10, label: 'Chef',    c1: 'var(--gold)',    c1dim: 'rgba(234,179,8,.18)',  c2: '#8a6a08' },
    { min: 6,  label: 'Senior',  c1: 'var(--purple)',  c1dim: 'rgba(168,85,247,.16)', c2: '#5b2f8a' },
    { min: 3,  label: 'Betjent', c1: 'var(--accent)',  c1dim: 'var(--accent-dim)',    c2: 'var(--accent-dim)' },
    { min: 0,  label: 'Kadet',   c1: 'var(--text-mid)', c1dim: 'var(--bg-elev)',      c2: 'var(--bg-elev)' },
];
function gradeTier(grade) {
    grade = tonum(grade);
    return GRADE_TIERS.find(t => grade >= t.min) || GRADE_TIERS[GRADE_TIERS.length - 1];
}
function tonum(v) { const n = parseInt(v, 10); return isNaN(n) ? 0 : n; }

function applyJobBadge(user) {
    const grade = user.liveGrade ?? user.grade ?? 0;
    const tier = gradeTier(grade);
    const root = document.documentElement.style;
    root.setProperty('--jb-c1', tier.c1);
    root.setProperty('--jb-c1-dim', tier.c1dim);
    root.setProperty('--jb-c2', tier.c2);

    $('#officer-rank').textContent = user.rank || 'Officer';
    $('#officer-grade-badge').textContent = `${tier.label} · G${grade}`;

    const avatar = $('#officer-avatar');
    if (user.profileImage) {
        avatar.style.backgroundImage = `url('${user.profileImage.replace(/'/g, '')}')`;
...
function renderSidenav(menus) {
    ...
    menus.forEach(m => {
        ...
        if (dividerBefore.has(m.id)) nav.appendChild(el('div', { className: 'nav-divider' }));
        ...
        if (isDev) btn.addEventListener('click', () => toast(`${m.label} er under udvikling.`, 'warning'));
        else btn.addEventListener('click', () => switchTab(m.id));
        nav.appendChild(btn);
    });
}
...
setInterval(() => {
    ...
    const dt = $('#duty-time'); if (dt) dt.textContent = fmtDuration(state.dutyBaseSec + live);
}, 1000);

// ─── ESC / CTRL+K SHORTCUTS ────────────────────────────────────
document.addEventListener('keydown', e => {
    if (e.ctrlKey && (e.key === 'k' || e.key === 'K')) {
        e.preventDefault();
        if (state.isLoggedIn && state.isOpen) {
            const q = $('#quick-search');
    ...
    if (!$('#pwchange-modal').classList.contains('hidden')) { return; }
...
window.addEventListener('message', e => {
    ...
    switch (m.action) {
        ...
        case 'warrantAlert':
            ...
            if (!m.data.removed && !m.data.edited) toast(`🚨 Ny efterlysning: ${m.data.reason || ''}`, 'error', 6000);
            break;
        case 'newDispatch':
            if (state.currentTab === 'dashboard') loadDashboard();
            if (state.currentTab === 'dispatch') Dispatch.handleNew(m.data);
            toast(`📡 ${m.data.code || 'Dispatch'}: ${m.data.description || ''}`, 'warning', 6000);
            break;
        case 'dispatchResolved':
            if (state.currentTab === 'dashboard') loadDashboard();
            if (state.currentTab === 'dispatch') Dispatch.handleResolved(m.data);
            break;
        case 'dispatchUpdated':
            if (state.currentTab === 'dispatch') Dispatch.handleUpdated(m.data);
            break;
        case 'updatePatrolList':
            if (state.currentTab === 'patrol') renderPatrolGrid(m.data);
            if (state.currentTab === 'dashboard') { const list = $('#dash-patrol'); if (list) renderDashPatrol(m.data); }
            break;
        case 'panicAlert':
...
function applyUserToUI() {
    ...
    $('#officer-name').textContent = state.user.name || state.user.username || '—';
...
$('#login-btn').addEventListener('click', async () => {
    ...
    if (!username || !password) { showLoginError('Indtast brugernavn og adgangskode.'); return; }
    ...
    if (!res || !res.ok) { showLoginError((res && res.error) || 'Netværksfejl.'); return; }

    state.isLoggedIn = true;
    state.user = res;
    state.user.username = username; // login-svaret indeholder ikke brugernavnet selv
    ...
    if (res.mustChangePw) showForcedPasswordChange();
    else showApp();
});
...
$('#pw-save').addEventListener('click', async () => {
    ...
    const err = $('#pw-error'); err.classList.add('hidden');
    ...
    if (!res || !res.ok) { err.textContent = (res && res.error) || 'Fejl.'; err.classList.remove('hidden'); return; }
    $('#pwchange-modal').classList.add('hidden');
...
function switchTab(name) {
    ...
    switch (name) {
        ...
        case 'laws':      loadLawsView(); break;
        case 'dispatch':  Dispatch.init(state.staticConfig); break;
...
async function loadDashboard() {
    ...
    const bw = $('#badge-warrants'), bd = $('#badge-dispatch');
    ...
    renderDashList('#dash-warrants', data.recentWarrants || [], w => ({
        title: `${w.firstname || '?'} ${w.lastname || ''}`, meta: fmtDate(w.created_at), sub: w.reason, cls: 'danger',
    }));
    renderDashList('#dash-dispatches', data.activeDispatches || [], d => ({
        title: `${d.code} · ${d.description}`, meta: d.location || fmtDate(d.created_at),
        cls: d.priority === 3 ? 'danger' : d.priority === 2 ? 'warning' : '',
    }));
    renderDashList('#dash-cases', data.recentCases || [], c => ({
        title: `${c.case_number} · ${c.title}`, meta: `${c.status} · ${fmtDate(c.created_at)}`, cls: c.status === 'closed' ? 'success' : '',
    }));
    renderDashList('#dash-arrests', data.recentArrests || [], a => ({
        title: `${a.firstname || '?'} ${a.lastname || ''}`, meta: `${a.officer_name} · ${fmtDate(a.created_at)}`,
        sub: `${a.total_fine || 0} kr · ${a.total_jail || 0} md`, cls: 'warning',
    }));
    renderDashList('#dash-topofficers', data.topOfficers || [], o => ({
        title: `${o.rank || ''} ${o.username || ''}`, meta: fmtDuration(o.total_sec), cls: 'purple',
    }));
...
function statusLabel(s) { return ({ 1: 'I tjeneste', 2: 'Ude af tjeneste', 3: 'Optaget', 4: 'Nødsituation' })[s] || '—'; }
...
async function loadOnlinePersons() {
    if (($('#persons-search').value || '').trim().length >= 2) return;
...
function renderPersonDetail(d) {
    ...
    const mugshot = m.mugshot_url ? `style="background-image:url('${escapeHtml(m.mugshot_url)}');"` : '';
...
function openWarrantModal(u) {
    const rlOpts = Object.entries(state.staticConfig.riskLevels).map(([k, v]) => `<option value="${k}">${v.label}</option>`).join('');
        ...
        <div class="modal-body">
            ...
            <div class="form-group"><label>Klip / billede</label>
                <div class="image-dropzone" id="w-dropzone" tabindex="0">
                    <div class="image-dropzone-hint"><span class="image-dropzone-icon">🖼</span><span>Klik her og indsæt billede med <strong>CTRL + V</strong></span></div>
    ...
    $('#w-save').addEventListener('click', async () => {
        ...
        closeModal();
        toast('Efterlysning udstedt.', 'success');
        setTimeout(() => openPerson(state.currentPerson), 300);
    });
}
...
// Delvis nummerplade-søgning (§21) — søg-mens-du-skriver med debounce,
// viser en klikbar resultatliste før man går videre til fuld detalje.
$('#vehicle-plate').addEventListener('input', debounce(async e => {
    const query = e.target.value.trim();
    const wrap = $('#vehicle-list-wrap');
    if (query.length < 2) { wrap.innerHTML = ''; return; }
    const list = await api('searchVehiclesList', { query }) || [];
...
function renderVehicleList(list) {
    ...
    const box = el('div', { className: 'vehicle-list' });
    for (const v of list) {
        const item = el('div', { className: 'vehicle-list-item', onclick: () => {
            $('#vehicle-plate').value = v.plate;
            wrap.innerHTML = '';
            searchVehicle();
        }});
        item.innerHTML = `
            <span class="vli-plate">${escapeHtml(v.plate)}</span>
            <div class="vli-info">
                <div class="vli-model">${escapeHtml(v.modelName || 'Ukendt model')}${v.flagged ? ' <span style="color:var(--danger);">🚨</span>' : ''}</div>
                <div class="vli-owner">${escapeHtml(v.owner || 'Ukendt ejer')}${v.stored ? ' · I garage' : ''}</div>
            </div>`;
...
function renderVehicleResult(r) {
    ...
    c.innerHTML = `
        <div class="vehicle-card">
            ...
            <h3 style="font-size:13px;margin-top:18px;">Markeringer</h3>
            ...
            <h3 style="font-size:13px;margin-top:24px;">Standsninger (seneste 20)</h3>
            <div style="margin-top:10px;">${stops}</div>
        </div>
    `;
    $$('#vehicle-result .vehicle-flag').forEach(b => b.addEventListener('click', () => b.classList.toggle('on')));
    $('#v-save').addEventListener('click', async () => {
        const flags = {};
        $$('#vehicle-result .vehicle-flag').forEach(b => { flags[b.dataset.flag] = b.classList.contains('on'); });
        flags.notes = $('#v-notes').value;
        await api('updateVehicleRecord', { plate: r.plate, flags });
        toast('Markeringer gemt.', 'success');
...
function flagBtn(key, label, on, isSuccess) {
    return `<div class="vehicle-flag ${on ? 'on' : ''} ${isSuccess ? 'success' : ''}" data-flag="${key}">
        <span>${label}</span><span class="flag-toggle"></span>
    </div>`;
}
...
$('#case-search').addEventListener('input', () => {
    clearTimeout(state.caseSearchTimer);
    state.caseSearchTimer = setTimeout(loadCases, 300);
});
$('#new-case-btn').addEventListener('click', openNewCaseModal);

async function loadCases() {
    const list = await api('getCases', { status: $('#case-filter').value, query: $('#case-search').value.trim() }) || [];
...
async function openCase(id) {
    ...
    if (!cs) return;
    const statusOpts = ['open', 'investigating', 'closed', 'archived'].map(s => `<option value="${s}" ${s === cs.status ? 'selected' : ''}>${s}</option>`).join('');
    openModal(`
        ...
        <div class="modal-body" style="max-height:70vh;">
            ...
            <h3 style="margin:10px 0;">Kommentarer</h3>
            ...
            <div style="display:flex;gap:6px;margin-top:8px;">
                <input type="text" id="c-newcomment" placeholder="Tilføj kommentar...">
    ...
    $('#c-save').addEventListener('click', async () => {
        await api('updateCase', { id, status: $('#c-status').value, description: $('#c-desc').value, priority: cs.priority, title: cs.title });
        closeModal();
...
// =============================================================
let warrantsCache = [];
$('#warrants-search').addEventListener('input', () => renderWarrants($('#warrants-search').value.trim().toLowerCase()));

async function loadWarrants() {
    warrantsCache = await api('getWarrants') || [];
    renderWarrants($('#warrants-search').value.trim().toLowerCase());
}
function renderWarrants(filter) {
    const list = !filter ? warrantsCache : warrantsCache.filter(w =>
        `${w.firstname || ''} ${w.lastname || ''}`.toLowerCase().includes(filter) || (w.reason || '').toLowerCase().includes(filter));
    const c = $('#warrants-grid'); c.innerHTML = '';
    if (!list.length) {
        c.innerHTML = `<div class="empty-state"><div class="empty-icon">✅</div><p class="muted">${filter ? 'Ingen match' : 'Ingen aktive efterlysninger'}</p></div>`;
        return;
    }
    for (const w of list) {
        ...
        card.innerHTML = `
            ...
            <div class="wc-meta"><span>${escapeHtml(w.issued_by)}</span><span>${fmtDate(w.created_at)}</span></div>
            <div class="wc-actions">
                <button class="btn btn-secondary" data-act="view">Profil</button>
                <button class="btn btn-secondary" data-act="edit">✎ Redigér</button>
                <button class="btn btn-secondary" data-act="toarrest">⚖️ Sigt</button>
        ...
        card.querySelector('[data-act="edit"]').addEventListener('click', () => openEditWarrantModal(w));
        ...
        card.querySelector('[data-act="remove"]').addEventListener('click', async () => {
            const ok = await confirmModal('Fjern efterlysning', `Fjern efterlysningen på ${w.firstname || '?'} ${w.lastname || ''}?`, { confirmLabel: 'Fjern' });
            if (!ok) return;
            await api('removeWarrant', { citizenid: w.citizenid });
            toast('Efterlysning fjernet.', 'success');
...
function openEditWarrantModal(w) {
    const rlOpts = Object.entries(state.staticConfig.riskLevels)
        .map(([k, v]) => `<option value="${k}" ${parseInt(k) === (w.risk_level || 0) ? 'selected' : ''}>${v.label}</option>`).join('');
    openModal(`
        <div class="modal-header"><h3>✎ Redigér efterlysning — ${escapeHtml(w.firstname || '')} ${escapeHtml(w.lastname || '')}</h3></div>
        <div class="modal-body">
            <div class="form-group"><label>Grund</label><textarea id="ew-reason" rows="3" maxlength="1000">${escapeHtml(w.reason || '')}</textarea></div>
            <div class="form-group"><label>Risikoniveau</label><select id="ew-risk">${rlOpts}</select></div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-ghost" onclick="closeModal()">Annullér</button>
            <button class="btn btn-primary" id="ew-save">Gem ændringer</button>
        </div>
    `);
    $('#ew-save').addEventListener('click', async () => {
        const reason = $('#ew-reason').value.trim();
        if (!reason) { toast('Angiv grund', 'error'); return; }
        await api('editWarrant', { id: w.id, reason, risk_level: parseInt($('#ew-risk').value) });
        closeModal();
        toast('Efterlysning opdateret.', 'success');
        setTimeout(loadWarrants, 200);
    });
}

// =============================================================
//  OPRET SIGTELSE (§15-§21)
// =============================================================
...
async function openArrestForm(u, warrant) {
    ...
    state.arrestForm = { citizenid: u.identifier, officers: [], images: [], seized: [], notes: '', charges: [], warrant_id: warrant ? warrant.id : null };
    ...
    if (warrant) {
        ...
        if (warrant.image) { state.arrestForm.images.push(warrant.image); renderArrestThumbs(); }
    }

    const draft = await api('getArrestDraft', { citizenid: u.identifier });
    if (draft && draft.payload) applyDraftToForm(draft.payload, draft.updated_at);
}
...
function closeArrestForm() {
    ...
    if (state.currentTab === 'persons' && cid) setTimeout(() => openPerson(cid), 100);
}
...
document.addEventListener('click', e => {
    if (!e.target.closest('.officer-search-wrap')) { const box = $('#af-officer-results'); if (box) box.classList.add('hidden'); }
});
...
$('#af-notes').addEventListener('input', () => { state.arrestForm.notes = $('#af-notes').value; scheduleArrestDraftSave(); });
...
async function saveArrestDraftNow() {
    if (!state.arrestForm) return;
    const payload = {
        notes: state.arrestForm.notes, charges: state.arrestForm.charges, officers: state.arrestForm.officers,
        images: state.arrestForm.images, seized_items: state.arrestForm.seized, warrant_id: state.arrestForm.warrant_id,
    };
    await api('saveArrestDraft', { citizenid: state.arrestForm.citizenid, payload });
    showDraftBanner(`💾 Kladde gemt kl. ${fmtClockNow()}`);
}
...
$('#af-submit').addEventListener('click', async () => {
    ...
    await api('createArrest', {
        citizenid: cid, charges: state.arrestForm.charges, total_fine: fine, total_jail: jail, notes,
        officers: state.arrestForm.officers, images: state.arrestForm.images, seized_items: state.arrestForm.seized,
...
async function loadPatrolPanel() {
    if (state.staticConfig && state.staticConfig.units) renderUnitGrid(state.staticConfig.units);
...
$$('.status-pill').forEach(p => p.addEventListener('click', async () => {
    ...
    if (s === 4) {
        const ok = await confirmModal('Aktivér nødsituation', 'Dette alarmerer alle betjente på tjeneste.', { confirmLabel: 'Aktivér', danger: true });
        if (!ok) return;
    }
...
function renderPatrolGrid(list) {
    ...
    const solo = [];
    for (const p of list) {
        if (p.vehicleNetId) { if (!groups.has(p.vehicleNetId)) groups.set(p.vehicleNetId, []); groups.get(p.vehicleNetId).push(p); }
        else solo.push(p);
    }
    ...
    for (const members of groups.values()) {
        ...
        card.innerHTML = `<div class="fc-vehicle"><span class="fc-icon">🚓</span>${escapeHtml(members[0].vehicleLabel || 'Køretøj')}</div>
            <div class="fc-members">${members.map(memberRow).join('')}</div>`;
        c.appendChild(card);
    }
    for (const p of solo) {
        const card = el('div', { className: 'fleet-card solo' });
        card.innerHTML = `<div class="fc-vehicle"><span class="fc-icon">👮</span>${escapeHtml(p.name)}</div>
...
// =============================================================
//  BØDESKEMA (kategoriseret, §12)
// =============================================================
async function loadCharges() {
    if (!state.chargesCache) state.chargesCache = await api('getCharges') || [];
    renderChargeCategoryNav();
    renderCharges();
}
$('#charges-search').addEventListener('input', renderCharges);
function renderChargeCategoryNav() {
    const nav = $('#charges-category-nav'); nav.innerHTML = '';
    const cats = [...new Set((state.chargesCache || []).map(c => c.category))].sort();
    const all = el('div', { className: 'category-nav-item' + (!state.chargesCategory ? ' active' : ''), onclick: () => { state.chargesCategory = null; renderChargeCategoryNav(); renderCharges(); } });
    all.innerHTML = `<span>Alle</span><span class="count">${(state.chargesCache || []).length}</span>`;
    nav.appendChild(all);
    for (const cat of cats) {
        const count = state.chargesCache.filter(c => c.category === cat).length;
        const item = el('div', { className: 'category-nav-item' + (state.chargesCategory === cat ? ' active' : ''), onclick: () => { state.chargesCategory = cat; renderChargeCategoryNav(); renderCharges(); } });
        item.innerHTML = `<span>${escapeHtml(cat)}</span><span class="count">${count}</span>`;
        nav.appendChild(item);
    }
}
function renderCharges() {
    const filter = $('#charges-search').value.trim().toLowerCase();
    const c = $('#charges-grid'); c.innerHTML = '';
    const list = (state.chargesCache || []).filter(x =>
        (!state.chargesCategory || x.category === state.chargesCategory) &&
    ...
    for (const ch of list) {
        ...
        div.innerHTML = `
            ...
            <div class="charge-vals"><span class="v-fine">${ch.fine} kr</span><span class="v-jail">${ch.jail} md</span></div>
...
// =============================================================
//  LOVBOG — 5 visninger (§5-§11): love / radio / rettigheder / korruption / våben
// =============================================================
const LAW_VIEW_CONTAINERS = { laws: '#lawview-laws', radio: '#radio-list', rights: '#rights-view', corruption: '#corruption-view', weapons: '#weapons-view' };
...
$('#laws-search').addEventListener('input', () => {
    clearTimeout(state.lawsSearchTimer);
    const q = $('#laws-search').value.trim();
    state.lawsSearchTimer = setTimeout(() => {
        if (state.lawsView === 'laws') loadLaws(q);
        else if (state.lawsView === 'radio') renderRadioCodes(q);
        // rights/corruption/weapons er statisk konfigurationsindhold — søgning filtreres ikke der.
    }, 300);
});

function loadLawsView() {
    for (const [view, sel] of Object.entries(LAW_VIEW_CONTAINERS)) {
        const container = $(sel);
        if (container) container.classList.toggle('hidden', view !== state.lawsView);
    }
    const searchBox = $('#laws-search');
    searchBox.style.display = (state.lawsView === 'laws' || state.lawsView === 'radio') ? '' : 'none';

    const q = searchBox.value.trim();
    if (state.lawsView === 'laws') loadLaws(q);
    else if (state.lawsView === 'radio') renderRadioCodes(q);
    else if (state.lawsView === 'rights') renderRightsView();
    else if (state.lawsView === 'corruption') renderCorruptionView();
    else if (state.lawsView === 'weapons') renderWeaponsView();
}

async function loadLaws(query) {
    const list = await api('getLaws', { query: query || '' }) || [];
    state.lawsCache = list;
    renderLawCategoryNav();
    renderLaws(filterLawsByCategory(list));
}
function filterLawsByCategory(list) {
    if (!state.lawsCategory) return list;
    return list.filter(l => l.book === state.lawsCategory);
}
function renderLawCategoryNav() {
    const nav = $('#laws-category-nav'); if (!nav) return;
    nav.innerHTML = '';
    const books = [...new Set((state.lawsCache || []).map(l => l.book))].sort();
    const all = el('div', { className: 'category-nav-item' + (!state.lawsCategory ? ' active' : ''), onclick: () => { state.lawsCategory = null; renderLawCategoryNav(); renderLaws(filterLawsByCategory(state.lawsCache || [])); } });
    all.innerHTML = `<span>Alle</span><span class="count">${(state.lawsCache || []).length}</span>`;
    nav.appendChild(all);
    for (const book of books) {
        const count = state.lawsCache.filter(l => l.book === book).length;
        const item = el('div', { className: 'category-nav-item' + (state.lawsCategory === book ? ' active' : ''), onclick: () => { state.lawsCategory = book; renderLawCategoryNav(); renderLaws(filterLawsByCategory(state.lawsCache || [])); } });
        item.innerHTML = `<span>${escapeHtml(book)}</span><span class="count">${count}</span>`;
        nav.appendChild(item);
    }
}
...
function renderRadioCodes(query) {
    ...
    for (const r of list) {
        const div = el('div', { className: 'law-card' });
        div.innerHTML = `<div class="law-head"><span class="law-title">${escapeHtml(r.code)}</span></div><div class="law-text">${escapeHtml(r.desc)}</div>`;
        c.appendChild(div);
    }
}

// ─── Sigtelser & rettigheder (§11) — standardformuleringer med {TIME} ──
function renderRightsView() {
    const data = (state.staticConfig && state.staticConfig.rightsPhrases) || {};
    const keys = Object.keys(data);
    if (!keys.length) { $('#rights-tabs').innerHTML = ''; $('#rights-grid').innerHTML = '<div class="empty-state"><p class="muted">Intet konfigureret</p></div>'; return; }
    if (!state.rightsTab || !data[state.rightsTab]) state.rightsTab = keys[0];

    const tabs = $('#rights-tabs'); tabs.innerHTML = '';
    for (const key of keys) {
        const tab = el('button', { className: 'rights-tab' + (state.rightsTab === key ? ' active' : ''), onclick: () => { state.rightsTab = key; renderRightsView(); } }, data[key].label || key);
        tabs.appendChild(tab);
    }

    const grid = $('#rights-grid'); grid.innerHTML = '';
    const entries = (data[state.rightsTab] && data[state.rightsTab].entries) || [];
    for (const entry of entries) {
        const card = el('div', { className: 'rights-card' });
        card.innerHTML = `
            <div class="rights-card-title">${escapeHtml(entry.title)}</div>
            <div class="rights-card-text">${escapeHtml((entry.text || '').replace('{TIME}', fmtClockNow()))}</div>
            <button class="btn btn-secondary btn-sm rights-copy-btn">📋 Kopiér</button>`;
        const btn = card.querySelector('.rights-copy-btn');
        btn.addEventListener('click', async () => {
            const text = (entry.text || '').replace('{TIME}', fmtClockNow());
            const ok = await copyText(text);
            if (ok) { btn.textContent = '✓ Kopieret'; btn.classList.add('copied'); setTimeout(() => { btn.textContent = '📋 Kopiér'; btn.classList.remove('copied'); }, 1600); }
            else toast('Kunne ikke kopiere.', 'error');
        });
...
// ─── Korruption (§9) ────────────────────────────────────────────
function renderCorruptionView() {
    const d = (state.staticConfig && state.staticConfig.corruption) || {};
    const view = $('#corruption-view');
    view.innerHTML = `
        <div class="info-section"><h3>Definition</h3><p>${escapeHtml(d.definition || '—')}</p></div>
        <div class="info-section"><h3>Tilladt</h3><ul class="check-list">${(d.allowed || []).map(x => `<li>${escapeHtml(x)}</li>`).join('') || '<li class="muted">—</li>'}</ul></div>
        <div class="info-section"><h3>Forbudt</h3><ul class="cross-list">${(d.forbidden || []).map(x => `<li>${escapeHtml(x)}</li>`).join('') || '<li class="muted">—</li>'}</ul></div>
        <div class="info-section"><h3>Procedure ved mistanke</h3><p>${escapeHtml(d.procedure || '—')}</p></div>
        <div class="info-section"><h3>Konsekvenser</h3><p>${escapeHtml(d.consequences || '—')}</p></div>`;
}

// ─── Våbenreglement (§10) ───────────────────────────────────────
function renderWeaponsView() {
    const d = (state.staticConfig && state.staticConfig.weapons) || {};
    const view = $('#weapons-view'); view.innerHTML = '';
    if (d.priorityNote) view.appendChild(el('div', { className: 'weapon-priority-note' }, `⚠ ${d.priorityNote}`));
    for (const cat of (d.categories || [])) {
        const section = el('div', { className: 'info-section' });
        section.innerHTML = `
            <div class="weapon-cat-title"><h3 style="margin:0;">${escapeHtml(cat.name)}</h3>${cat.priority ? '<span class="pill pill-danger">Dødeligt</span>' : '<span class="pill pill-success">Ikke-dødeligt</span>'}</div>
            ${(cat.entries || []).map(e => `<div style="margin-bottom:10px;"><strong style="font-size:12.5px;">${escapeHtml(e.title)}</strong><p style="font-size:12.5px;color:var(--text-mid);margin-top:3px;line-height:1.5;">${escapeHtml(e.text)}</p></div>`).join('')}`;
        view.appendChild(section);
    }
    if (d.storage) view.appendChild(el('div', { className: 'info-section' }, el('h3', {}, 'Opbevaring'), el('p', {}, d.storage)));
}

// =============================================================
//  ACCOUNTS (boss panel)
// =============================================================
$('#accounts-search').addEventListener('input', () => renderAccounts($('#accounts-search').value.trim().toLowerCase()));
async function loadAccounts() {
    state.accountsCache = await api('getRecentAccounts') || [];
    renderAccounts($('#accounts-search').value.trim().toLowerCase());
}
function renderAccounts(filter) {
    const list = !filter ? (state.accountsCache || []) : (state.accountsCache || []).filter(a =>
        (a.username || '').toLowerCase().includes(filter) || (a.badge_number || '').toLowerCase().includes(filter));
    const tb = $('#accounts-tbody'); tb.innerHTML = '';
    if (!list.length) { tb.innerHTML = '<tr><td colspan="9"><div class="empty-state"><p class="muted">Ingen konti fundet</p></div></td></tr>'; return; }
    for (const a of list) {
        const tr = el('tr');
        tr.innerHTML = `
        ...
        tr.querySelector('[data-act="ban"]').addEventListener('click', async () => {
            ...
            if (newSt === 'banned') {
                const ok = await confirmModal('Bekræft ban', `Ban kontoen ${a.username}?`, { confirmLabel: 'Ban' });
                if (!ok) return;
            }
            const r = await api('setAccountStatus', { username: a.username, status: newSt });
            if (r && r.ok) { toast('Status opdateret', 'success'); loadAccounts(); }
...
// ─── SETTINGS · Change password (Settings/Konti §29 — MÅ IKKE ødelægges) ──
...
// ─── QUICK SEARCH / GLOBAL SEARCH (Ctrl+K, topbar) ─────────────
...
document.addEventListener('click', e => { if (!e.target.closest('.quick-search')) $('#quick-search-results').classList.add('hidden'); });
