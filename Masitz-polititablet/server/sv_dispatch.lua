...
-- ─── TAG / GÅ AF OPKALD (§25) ──────────────────────────────────
-- Enhver logget-ind betjent kan tage/forlade et opkald (samme
-- rettighedsniveau som at oprette ét) — det er ikke en destruktiv
-- handling, kun `resolveDispatch` (luk sagen) kræver den højere
-- resolveDispatch-permission.
local function loadDispatchUnits(dispatchId)
    local row = MySQL.single.await(
        "SELECT units FROM polititablet_dispatches WHERE id=? AND status!='resolved'",
        { dispatchId }
    )
    if not row then return nil end
    local ok, units = pcall(json.decode, row.units or '[]')
    if not ok or type(units) ~= 'table' then units = {} end
    return units
end

RegisterNetEvent('kcmdt:takeDispatch', function(dispatchId)
    local src = source
    local sess = KCS.requireSession(src); if not sess then return end
    if not KCS.spamCheck(src) then return end
    dispatchId = tonumber(dispatchId); if not dispatchId then return end

    local units = loadDispatchUnits(dispatchId)
    if not units then return end

    local already = false
    for _, u in ipairs(units) do
        if u.identifier == sess.identifier then already = true; break end
    end
    if not already then
        units[#units + 1] = { identifier = sess.identifier, name = sess.rawName, unit = sess.unit }
    end

    MySQL.update.await(
        "UPDATE polititablet_dispatches SET units=?, status='active' WHERE id=?",
        { json.encode(units), dispatchId }
    )
    KCS.broadcastMDT('kcmdt:dispatchUpdated', { id = dispatchId, units = units, status = 'active' })
end)

RegisterNetEvent('kcmdt:dropDispatch', function(dispatchId)
    local src = source
    local sess = KCS.requireSession(src); if not sess then return end
    if not KCS.spamCheck(src) then return end
    dispatchId = tonumber(dispatchId); if not dispatchId then return end
    ...
    local out = {}
    for _, u in ipairs(units) do
        if u.identifier ~= sess.identifier then out[#out + 1] = u end
    end

    MySQL.update.await('UPDATE polititablet_dispatches SET units=? WHERE id=?', { json.encode(out), dispatchId })
    KCS.broadcastMDT('kcmdt:dispatchUpdated', { id = dispatchId, units = out })
end)
