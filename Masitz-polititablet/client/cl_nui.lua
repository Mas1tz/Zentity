...
RegisterNUICallback('editWarrant', function(data, cb)
    TriggerServerEvent('kcmdt:editWarrant', data.id, data.reason, data.risk_level); cb({ ok = true })
end)
...
-- Delvis nummerplade-søgning (§21) — resolver modelnavn client-side pr.
-- resultat, samme mønster som searchVehicle ovenfor.
RegisterNUICallback('searchVehiclesList', function(data, cb)
    local res = safeAwait('kcmdt:searchVehiclesList', data.query) or {}
    for _, v in ipairs(res) do
        if v.vehicleRaw then
            local ok, parsed = pcall(json.decode, v.vehicleRaw)
            if ok and type(parsed) == 'table' and parsed.model then
                local hash = type(parsed.model) == 'number' and parsed.model or GetHashKey(parsed.model)
                local disp = GetDisplayNameFromVehicleModel(hash)
                local lbl  = GetLabelText(disp)
                v.modelName = (lbl ~= '' and lbl ~= 'NULL') and lbl or disp
            end
            v.vehicleRaw = nil
        end
    end
...
-- ═══════════════════════════════════════════════════════════
--  DISPATCH
--  sv_dispatch.lua eksisterede allerede fuldt funktionsdygtig
--  (koordinater verificeret server-side, permission-tjek,
--  broadcast til alle online MDT-betjente) — der manglede blot
--  disse callbacks for at koble NUI'en på.
-- ═══════════════════════════════════════════════════════════
RegisterNUICallback('getDispatches', function(_, cb)
    cb(safeAwait('kcmdt:getDispatches'))
end)
RegisterNUICallback('createDispatch', function(data, cb)
    TriggerServerEvent('kcmdt:createDispatch', data); cb({ ok = true })
end)
RegisterNUICallback('resolveDispatch', function(data, cb)
    TriggerServerEvent('kcmdt:resolveDispatch', data.id); cb({ ok = true })
end)
RegisterNUICallback('takeDispatch', function(data, cb)
    TriggerServerEvent('kcmdt:takeDispatch', data.id); cb({ ok = true })
end)
RegisterNUICallback('dropDispatch', function(data, cb)
    TriggerServerEvent('kcmdt:dropDispatch', data.id); cb({ ok = true })
end)
-- Bruges af dispatch-kortets "auto" dag/nat-tilstand (§26). Rent lokalt
-- klient-kald — ingen serverkommunikation nødvendig for spillets ur.
RegisterNUICallback('getGameClock', function(_, cb)
    cb({ hour = GetClockHours() })
end)
