...
RegisterNetEvent('kcmdt:warrantAlert', function(data)
    ...
    -- 'removed'/'edited' er stille opdateringer (NUI'en genindlæser bare
    -- listen) — kun en reelt NY efterlysning skal give den store 🚨 toast.
    if not data.removed and not data.edited then
        lib.notify({
...
RegisterNetEvent('kcmdt:dispatchUpdated', function(data)
    if KCC.isOpen then SendNUI('dispatchUpdated', data) end
end)
